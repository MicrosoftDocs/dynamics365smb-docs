---
title: "Contract Gain-Loss (Groups) Overview"
ms.custom: na
ms.date: "03-03-2017"
ms.reviewer: na
ms.suite: na
...
ms.topic: "article"
helpviewer_keywords: 
  - "service contracts, gain/loss by group"
ms.assetid: b79275cd-ffb1-4ab8-b719-7724de305fd9
...
The **Contract Gain\/Loss \(Groups\)** matrix shows a scrollable summary of contract gain\/loss entries, based on service contract groups.  
 ...
 You fill in the **Contract Gain\/Loss \(Groups\)** window to determine what will be shown in the matrix.  
  
 You can specify which contract groups are included in the window by setting filters in the **Contract No. Filter** field. If you do not set any filters, the window will include information about all service contract groups.  
 ...
 You choose **Show Matrix** to view the matrix. The first three columns contain the time intervals and the total gain\/loss amount for each interval, and the remaining columns contain the total gain\/loss amount for each contract group for the same time intervals.  
...
## See Also  
 [\($ N\_6066 Contract Gain\-Loss \(Service Contract Groups\) $\)](../Topic/\($%20N_6066%20Contract%20Gain-Loss%20\(Service%20Contract%20Groups\)%20$\).md)