...
## Bin Adjustment in Basic Warehouse that Use Bins Only  
 If you are not using directed put\-away and pick at the location, you can change the quantity in a bin with the  **\($ N\_40 Item Journal $\)** window. Posting the item journal adjusts the quantity in the item ledger entries and the related warehouse entries.  
...
> [!NOTE]  
>  The entries in the adjustment bin may originate from the warehouse item journal, the warehouse physical inventory journal, the item journal, or a number of other documents that indicate changes in warehouse inventory. For more information, see [\($ R\_7320 Warehouse Adjustment Bin $\)](../Topic/\($%20R_7320%20Warehouse%20Adjustment%20Bin%20$\).md).  
