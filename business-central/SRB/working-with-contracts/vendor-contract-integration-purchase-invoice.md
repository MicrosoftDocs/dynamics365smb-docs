...
This enables automated, accurate billing and reduces manual data entry.

## Prerequisites

- Access to the Subscription Billing or Vendor Contract module.
- Appropriate permissions to process purchase invoices and vendor contracts.

## Retrieve Vendor Contract Lines in Purchase Invoice
...
The accountant receives an invoice from a supplier that belongs to a vendor subscription contract. The accountant wants to compare the invoice with the contract and then process it without having to leave the purchase invoice. Starting from the purchase invoice, he wants to be able to call up the contract elements (lines) in the purchase invoice. He must be able to adjust the billing period and the amount. This procedure ensures that the contract is updated correctly.
...
2. **Worksheet Get Vendor Subscription Contract Lines**
   ...
   - Fields: **Contract No., Your Reference, Type, No., Description Subscription, Description Subscription Line, Next Invoice Date, Billing to Date (editable), Vendor Invoice Amount (editable), Quantity, Price, Amount, Calculation Base Amount, Calculation Base %, Billing Base Period, Billing Rhythm**
   - All fields except those stated are not editable.

3. **Details**
   - “Billing to Date” is calculated and preset based on “Next Invoice Date” and “Billing Rhythm” from the vendor subscription contract line.
...
4. **Processing**
   - On confirmation (OK), billing lines are created (analogous to “Create Invoice” in vendor subscription contract) with the relevant dates and amounts.
...
## Assign Purchase Invoice Line to Vendor Contract Line
...
2. **Worksheet Link Purchase Line with Contract Line**
   ...
   - Only “Selected” and “Billing to Date” are editable, amounts are taken from the corresponding invoice lines and are therefore not editable.

3. **Details**
   - “Billing to Date” is preset based on “Next Invoice Date” and “Billing Rhythm” from the vendor subscription contract line.
