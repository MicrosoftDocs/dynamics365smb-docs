...
## See where a general ledger account is used in setup tables 

When you set up [!INCLUDE [prod_short](includes/prod_short.md)], you filled in setup tables (for example, the posting group tables) to specify which G/L account is used as posting accounts for which types of transactions. 

The **Chart of Accounts** page and **G/L Account** card both include a *Where-Used* action that opens a window to get an overview of which setup tables are using the given G/L account.

Each line in the window refers to one line in a setup table. The window can list several lines from the same setup table if, for example, the account is used for more than one posting group code in a posting setup table.

You can even see details for a single line by choosing the *Show Details* action on the window.
