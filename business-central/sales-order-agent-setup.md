...
1. On the right side of the page, select the **Go to next card** arrow (6), and then choose how the agent helps with inquiries, quotes, and orders. There are several options:
    ...
    |Create sales documents|When on, the agent creates sales quotes and orders from email inquiries based on the remaining options in this table.<br><br>When off, the agent handles only handles email communication and doesn't crete quotes or orders. The remaining options are irrelevant and can't be set.|On|
