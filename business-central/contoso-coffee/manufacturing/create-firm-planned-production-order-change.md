...
Eduardo, the production planner at Contoso Coffee, must create a new production order for 10 units of the item **SP-SCM1009, Airpot** that must be due on April 28. Eduardo backward schedules the order and confirms that they can start the order on April 27.  

Shortly after the task is complete, Eduardo is asked to increase the order to 50 units. On doing so, the backward scheduling functionality pushes the order start date too early. So Eduardo forward schedules the order from April 23 in order to determine a more realistic finish date.  
...
2. Change the production order's quantity to 50 units and schedule the order.  
    ...
    1. On the **Lines** FastTab of the **Production BOM**, select the recently added line, and then, in the **Quantity** field, enter *50*.  
        ...
        The start date is now pushed back to April 20. This date isn't acceptable for Eduardo.
...
> [!NOTE]
> Based on the year, April 23 might fall on a weekend. In such cases, the start date is automatically shifted to the next working day, for example April 25. The ending date and due date might also change for the same reason.
