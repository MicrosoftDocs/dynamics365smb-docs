...
2. On the **Data Export Record Source** page, fill in the fields as described in the following table.  
    ...
    > [!NOTE]  
    >  INDEX.XML file contains table and field names in English based on tables and fields names. Table and field names are written under **<Name>** tag and Captions are written under **<Description>** tag in index.xml.   
    
    > [!NOTE]  
    >  Table and field names will be written as is, i.e. with spaces, dots, parentheses etc. The only characters that are removed from the names are **&**, **'** (single quote), **"** (double quote), **<** (less-than sign), **>** (greater-than sign).   
