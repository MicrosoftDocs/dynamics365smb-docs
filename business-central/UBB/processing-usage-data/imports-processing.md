...
|---------|---------|---------|
|Subscription ID     | Links the usage record to a subscription line, through the supplier's [usage data supplier reference](../masterdata/references.md).        | Required. Without it, the record can't be matched to a subscription line, and processing fails.       |
|Billing Period Start Date     | Start of the period the usage record charges for. Must be on or after the start date of the matched subscription line.        | Required.       |
|Billing Period End Date     | End of the period the usage record charges for.        | Required.       |
|Quantity     | The usage quantity that pricing is calculated from.        | Required to bill a non-zero amount.       |
|Customer ID     | Identifies the customer at the supplier.        | Required only if you turn on **Create Customers** in the [Generic Import Settings](../masterdata/suppliers.md#settings-on-the-supplier) for the supplier.       |
|Price / Cost     | The supplier's unit price for the usage.        | Required only if you turn on the **Unit Price from Import** toggle in the [Usage Data Suppliers](../masterdata/suppliers.md) settings.       |
|Product ID     | The supplier's identifier for the product or service the usage relates to.        | Optional. Used only to detect and connect available service objects; it doesn't affect billing.       |
|Currency     | The currency the usage data was recorded in.        | Optional. If left blank, [!INCLUDE [prod_short](../../includes/prod_short.md)] uses the subscription contract's currency.       |

> [!NOTE]
> If a required field is missing, or doesn't resolve to an existing subscription line, the record isn't processed. Use the lookup in the **No. of Imported Line Errors** field on the **Usage Data Imports** page to review and correct these records, and then run the processing step again.
