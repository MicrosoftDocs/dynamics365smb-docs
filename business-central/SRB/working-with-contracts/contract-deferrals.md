...
Contract deferrals from customer contracts are used for deferring revenues in sales invoices.
...
Customer contract deferrals can be accessed in the **Customer Contract Card** page using the **Customer Contract Deferrals** action. The page that opens displays the following information in detail:
...
Analogous to the customer contract, the G/L accounts to post to are determined using posting groups for the respective business transaction. In the vendor contract, this is the combination of the vendor's general business posting group in the Contract Header and the general product posting group of the item.

By default, the account used is the one stored in the **Vendor Contract Deferral Account** field in the **General Posting Setup**. However, if the **Without Contract Deferrals** indicator is set to *Yes* in the vendor contract, the invoice amount is immediately posted as a cost to the account stored in the **Vendor Contract Account** field. In this case, contract deferrals aren't created.

### Calling contract deferrals from vendor contracts

Vendor contract deferrals can be accessed in the **Vendor Contract Card** page using the **Vendor Contract Deferrals** action. The page that opens is the same as the one for [Customer contract deferrals](#calling-contract-deferrals-from-customer-contracts).
...
The batch report **Contract Deferrals Release** is used to release invoice deferrals. This report runs through all contract deferrals (customer and vendor), filters using **Posting Until Date** from the report request page to the **Posting Date** and **Document Posting Date** fields, and reposts the items thus found from the accrual account to the cost or revenue account. The release takes place for debit-side and credit-side contract accruals at the same time. The **Posting Date** field from the report request page is used as the *Posting Date* for the closure. At the same time, the **Released** field in the contract deferrals is set to *Yes*.
It is therefore recommended to trigger the release of contract deferrals on a monthly basis (for example, at the end of the month). In the future, it will also be possible to perform the release of contract deferrals automatically via the job queue. To learn more, go to [Job queue](../setup/job-queue.md).
...
> [!NOTE]
> * If deferrals were created by a contract invoice, they will automatically be fully released on the **Posting Date of Credit Memo** when the contract invoice is credited.
...
For the control of the deferrals and for the submit for auditors, you can export the deferrals to Excel or to print them. For this purpose, you can use the **Customers Contract Deferrals Analysis** and **Vendors Contract Deferrals Analysis** (*Alt+Q*) calls.
