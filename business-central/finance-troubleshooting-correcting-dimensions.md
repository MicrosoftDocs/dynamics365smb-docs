...
If an incorrect number is assigned, when you open the General Ledger Entries page we will display a notification that includes a link to the Error Messages page. From the Error Messages page, you can choose the link in the **X** field to run a report that will correct global dimension number assignments in the Dimension Set Entry table. Alternatively, you can also search for **Update Global Dimension No. for Dimension Set Entries** to run the report.
