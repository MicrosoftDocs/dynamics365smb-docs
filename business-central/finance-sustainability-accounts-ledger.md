...
The chart of sustainability accounts (CoSA) is a structured list that you can use to record emissions data. It's a framework that organizes sustainability accounts based on their attributes, such as the scope or other groupings. Each account is typically assigned a unique code for easy reference and tracking. The CoSA has the same structure as a traditional chart of accounts, but is customized to monitor sustainability-related data and metrics.

You can add sustainability account categories and sustainability account subcategories to define how the system behaves. Categories and subcategories let you select dedicated emissions to track, emission factors, formulas, and similar configurations.
...
> [!IMPORTANT]
> The **Indent** action overwrites the value of all fields for **End-Total** accounts. Therefore, if you entered definitions in the **Totaling** field for **End-Total** accounts before you ran the **Indent** action, you must enter them again after you run it.
...
You must add a sustainability account category to each sustainability account. You can select emission scopes, dedicated emissions to track, formulas, and similar configurations.
...
2. On the **Sustainability Account Categories** page, edit the list or create a new category. To create a new category, select the **New** action.
...
6. In the **Calculation Foundation** field, select the calculation foundation (formula) to use for emission calculations if you don't know the accurate emission amount.

    > [!NOTE]
    > If the formulas in the **Calculation Foundation** field don't meet your needs, you can extend the field and add more calculations for use in the sustainability journals.
...
2. On the **Sustainability Account Subcategories** page, edit the list or create a new category. To create a new category, select the **New** action.
3. Fill in the **Code** and **Description** fields.
...
> [!NOTE]
> The **Import Data** and **Import From** fields are intended for potential integration with external systems that collect emission factors. However, in **2024 release wave 1**, these fields can't be used as a feature by default.

You can have more than one account subcategory related to a sustainability account, but only one can be the default value. You must set the default value for the account every time if you want to use it for posting.  

> [!TIP]
> For example, if you want to use one account for business cars, but you have different car types with different emission factors. You can set up an account and create account subcategories for cars with different emission factors. When you work in your sustainability journal, you can easily change your account subcategory based on the car type.  
