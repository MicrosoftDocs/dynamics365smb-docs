...
If you have situation like where there’s no Data Subject table relationship, for example the Profile answers table, you need to: 
-	Consider if you really want / need to export (supplemental) data that is unrelated to contact (has no direct relationship to it) 
-	Add this table and relationship manually to Rapid Start package and export it directly from Rapid Start package – that’s why we generate Rapid Start package for you, so you can tweak it like in situations like this.
