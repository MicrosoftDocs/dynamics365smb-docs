...
description: Provides information about the fixed asset inventory feature and related enhancements for Russia.
author: DianaMalina
ms.topic: article
ms.search.keywords: fixed asset inventory, Russia
ms.date: 07/11/2025
...
- Print the forms with inventory lists that show all inventoried fixed assets, and lists that show the fixed assets with differences only in quantities or amounts.
...
1. Choose the **Calculate FA** action. The request form of a report that makes fixed asset inventory lists is displayed.
1. On the **Fixed Asset** FastTab, filter the fixed assets.
   ...
   |:-|:-|--|
   | :--------------------------------------- | :----------------------------------------------------------- |
   | **Fixed Asset Journal Template** | Select the fixed asset journal template to work with the fixed assets list during the inventory auditing process. It's filled by default with the fixed asset journal template. |
   | **Depreciation Book Code** | Select the depreciation book with the records, which is calculated by quantities and amounts. |
...
1. Place the columns of the fixed asset journal so that you can view calculated and actual quantities and amounts. They're reflected in the following fields:
