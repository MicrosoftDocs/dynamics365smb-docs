...
When the QR-bill is scanned or imported to the purchase document, the amount, payment reference, and other information from the QR-bill are added. This data is used for validation before posting the purchase document. Posting will be blocked if the amount on the order or invoice does not match the amount on the QR-bill. Validation also happens when you scan or import the QR-bill. But if the user uploads the QR-Bill that does not contain the **Amount** (i.e. the decoded QR-Bill has a blank line where amount should be), after the QR-Bill is uploaded, a **QR-Bill** section will be shown on the **Purchase Invoice** page. The field **Amount** of the **QR-Bill** section will be allowed to edit, and a user sets the necessary amount to this field. If an uploaded QR-Bill has non-zero **Amount**, it is automatically set to the **Amount** field of the **QR-Bill** section but the field is still allowed to edit if necessary.  

If the payment reference is already in use on a vendor ledger entry for a vendor, an error displays. Vendors cannot issue multiple QR-bills with the same payment reference. Similarly, an error displays if the QR-bill and payment reference have already been imported to an open purchase document.
