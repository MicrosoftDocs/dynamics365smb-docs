...
description: Learn how Business Central supports SII and the output of the various types that are used for invoices and credit memos in the Spanish version.
author: SorenGP
...
For information about how to set up [!INCLUDE [prod_short](../../includes/prod_short.md)] for SII, see [Set Up SII for VAT Reporting in the Spanish Version](sii-setup.md).  
...
|F2|Simplified invoice (ticket)|Implemented as for F1, except a non-existing block is called `Contraparte` and other nodes are called `ImporteTotal` and `Macrodato`.|
