...
> [!NOTE]
> If you are using the demonstration database to try out [!INCLUDE[prodshort](includes/prodshort.md)] on-premises, and you want to explore retention policies, there are a few things you need to do. The Cronus demonstration company does not contain tables that you can use with retention policies, so you need to add them. To do that, in your demonstration database, create a new, blank company and then import the NAV17.0.W1.ENU.STANDARD.rapidstart package. The setup data for retention policies will be available in the new company.
