...
Your general ledger is set up to use your local currency (LCY), but you can set it up to also use another currency with a current exchange rate assigned. By designating a second currency as a so-called additional reporting currency, [!INCLUDE[d365fin](includes/d365fin_md.md)] will automatically record amounts in both LCY and this additional reporting currency on each G/L entry and other entries, such as VAT entries. When G/L entry amounts are calculated in an additional reporting currency, [!INCLUDE[d365fin](includes/d365fin_md.md)] uses the information from the **Currency Exchange Rates** page to find the relevant exchange rate.
