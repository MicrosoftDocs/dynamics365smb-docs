...
You can set up synchronization between ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]-->.  
 ...
 The synchronization framework provides default mapping between the ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]--> contacts, salesperson, and to-dos, and Microsoft Outlook contacts, tasks, and meetings. You can synchronize the entities either manually or automatically at set intervals.  
  
> [!IMPORTANT]  
>  You can create new a salesperson contact in Outlook, but that contact will not be synchronized to ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]--> as a salesperson.  
  
 One of the strengths of the ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]--> Outlook synchronization feature is that it can be customized without adding code. It is fairly simple to add more fields in order to optimize synchronization.  
  
 The default setup contains mapping for five different ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]--> entities:  
...
-   In ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]-->, make sure that the following is true:  
...
-   In Outlook, make sure that the following is true:  
    ...
    -   ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]--> Synchronization buttons are displayed in the Add-ins tab on the Outlook ribbon.  
  
    -   The connection between ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]--> and Outlook is working. To test the connection, choose the **Settings** button, and then on the **Connection** tab, choose **Test Connection**.  
  
## Story  
 Michael is an account manager for ADD INCLUDE<!--[!INCLUDE[demoname](../../includes/navnow_md.md)]-->. His coworker, David, is a sales representative who works mainly in the field and uses Outlook to track contacts, tasks, and appointments. Because Michael and David share information, they synchronize their information regularly throughout the day. This enables Michael to set appointments for David, while also keeping track of David's activities in the field.  
  
 In this scenario, Michael creates an appointment for David to meet with an existing customer. When David synchronizes, he sees the meeting that Michael has created. He does not think that he can drive to the location in time, so that he moves the meeting to another day in his Outlook calendar. He synchronizes again and the changes are updated in ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]-->, so that Michael can track the change.  
  
 Michael attempts to call the contact, but the telephone number is incorrect because someone else has entered a new number in Outlook and not yet synchronized with ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]-->. Michael reviews the conflict details, determines that the problem is related to the telephone number, and resolves the conflict.  
  
 Michael also realizes that he has to add some additional fields to the entities that are synchronized. He adds the territory code fields as user-defined fields to the company contact information so that they will be displayed in both Outlook and ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]-->.  
  
## Performing a Synchronization  
 First, Michael sets an appointment for David to meet with a customer by creating a to-do in ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]-->, and then synchronizing with Outlook, so that the appointment is displayed on the team calendar.  
...
9. In Microsoft Outlook, choose **Calendar** to open the calendar view, and then in the My Calendars list, open your ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]--> calendar.  
  
10. On the ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]--> toolbar, choose the **Synchronize** button to activate synchronization.  
  
     The synchronization runs, and you can see the appointment that you created on the shared calendar.  
  
 David checks the synchronized calendar for ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]--> updates. He sees the appointment and adjusts the time to fix the conflict.  
  
#### To change a meeting request in ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]--> and synchronize with Outlook  
  
1.  In your Outlook calendar, move the appointment created in ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]--> to another day, and then choose the **Synchronize** button to activate synchronization again.  
  
2.  Now, open ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]--> and locate the contact card for CT100006.  
...
1.  In Outlook, choose the **Settings** button to open the **Settings** dialog box.  
     ...
     The **Settings** dialog box presents several options for resolving conflicts, including options to automatically replace Outlook items with ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]--> records with Outlook items.  
  
2.  Select the **Resolve conflicts manually**, and close the **Settings** dialog box.  
  
3.  In ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]-->, open the contact card for **A. Gibson's Law Firm**, CT100006, and add the telephone number **1234567**.  
  
4.  Open Outlook and find the contact for **A. Gibson's Law Firm**.  
  
5.  Change the telephone number to **1224567**, so that it differs from the number that you entered in ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]-->.  
...
1.  Open ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]-->.  
  
2.  In the **Search** box, enter **Outlook Synch Entities**, and then choose the related link.  
  
     The **Outlook Synch. Entity** window contains the definition of the mapping between contacts in ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]--> and contacts in Microsoft Outlook.  
...
6.  Because there are no comparable Outlook property to map this to, select the **User Defined** check box for the **Territory Code** field. Choose the **OK** button.  
     ...
     Because entries in the Change Log are used to identify changes in ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]-->, you must add this field to the change log as well.  
...
15. When the synchronization is finished, in Microsoft Outlook, open the new contact that you created in ADD INCLUDE<!--[!INCLUDE[navnow](../../includes/navnow_md.md)]-->.  
