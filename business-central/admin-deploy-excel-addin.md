...
| The add-in communicates using the API v2.0 for Dynamics 365 Business Central, any limitations of this API is automatically inherited. An example limitation is if you try to edit a list and the underlying card uses a confirmation dialog in its AL logic e.g. its validation logic. | Sometimes there is nothing to do because it is just a design choice that the user should explicitly confirm the change. If the confirmation is negligible when using Edit in Excel, then you can wrap the Dialog Confirmation call in a if-conditional that checks if the client type is different from ODataV4 like this `if SESSION.CurrentClientType() <> ClientType::ODataV4 then` | Notice that there might be other clients that you want to remove the confirmation dialog from, e.g. OData and SOAP |
