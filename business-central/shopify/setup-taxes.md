...
In USA or Canada, customers expect to see prices without taxes, and taxes added at check-out. So the **All prices include tax** field is usually not selected. In this case, *$100* defined in the product card represents the price without tax, and at the checkout stage, taxes will be added on top of that price.  

To support the scenario when **All prices includes tax** is selected on the [!INCLUDE[prod_short](../includes/prod_short.md)] side, you must fill in the **Customer Template Code** field in **Shopify Shop Card** page with the template that has following fields defined:  
...
> [!NOTE]
> If you configured the Shopify connector to create customers automatically, you may need more fields in your template, such as **Customer Posting Group**. If you used default customer for imported orders, make sure this customer has same fields populated. You still need to fill in **Customer Template Code** as specified above, because the **Prices including Tax**/**Prices including VAT** field in the created sales document doesn't depend on the customer, but on the **Customer Template** from the Shopify Shop Card or Customer template per country.

## Sell internationally

In this section, we'll explore settings for scenarios, when you are required to collect taxes when selling to another country. For example, you are selling to other countries inside EU.  

Currently, the **Shopify Connector** extension supports export of one price only. Shopify automatically applies local taxes, currencies, rounding. The **All prices include tax** toggle will have the effects that is described in the following subsections.
...
Because different countries have different requirements for whether tax is included in the shown price or not, you can swich on [dynamic tax-inclusive pricing](https://help.shopify.com/en/manual/markets/pricing/dynamic-tax-inclusive-pricing) in Shopify.   

Just select **Include or exclude tax based on your customer's country** in the **Other Markets - Preferences** section of the [**Markets**](https://www.shopify.com/admin/settings/markets) settings in your **Shopify admin**.  
...
Different EU countries have different tax local tax rates. However, if you are located in EU and sell to other EU countries, you can use local tax rate in some cases.  

Check the **Collect VAT** in the **European Union** section of the [**Taxes and duties**](https://www.shopify.com/admin/settings/taxes) settings in your **Shopify admin**.
...
In the following example, the **All prices include tax** field is selected. The price in the product card is set to *1200*.	
...
In example below the All prices include tax toggle is enabled. Price in Product card is set to 1200.	
...
Shopify ignores the tax rate in the foreign country when calculating final prices and uses domestic tax rate.
...
* Set *Yes* if **All prices include tax** is enabled and **Include or exclude tax based on your customer's country** is disabled.
* Set *No* if **All prices include tax** is disabled and **Include or exclude tax based on your customer's country** is disabled.
...
> [!Note]
> The setting of the **Prices Including VAT** field comes from the template, not from the specific customer. That's why it is important to have customer template defined.
