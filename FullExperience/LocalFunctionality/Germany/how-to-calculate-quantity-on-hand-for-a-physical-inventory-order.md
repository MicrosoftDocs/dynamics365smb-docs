...
4.  To calculate the expected quantity in all physical inventory order lines, on the **Actions** tab, in the **Functions** group, choose **Calculate Qty. Expected**, and then choose **All Lines**.  
 ...
 If [!INCLUDE[navnow](../../ApplicationDesign/includes/navnow_md.md)] has already calculated the expected quantity, it shows this by placing a check mark in the [Qty. Exp. Calculated](../../LocalFunctionalityForMicrosoftDynamicsNav2016/Austria/-$-t_5005351_51-qty.-exp.-calculated-$-.md) field in the inventory order line.  
...
>  The process of taking the inventory and recording it in the physical inventory recording is independent from the calculation of the expected quantities.  
 ...
 You must calculate the expected quantities for all physical inventory order lines of a physical inventory order before you can set the [Status](../../LocalFunctionalityForMicrosoftDynamicsNav2016/Austria/-$-t_5005350_11-status-$-.md) field to **Finished**. This is because [!INCLUDE[navnow](../../ApplicationDesign/includes/navnow_md.md)] compares the expected and the recorded quantities and calculates the differences for posting.  
