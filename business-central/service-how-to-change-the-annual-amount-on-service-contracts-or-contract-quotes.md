...
ms.topic: how-to
ms.devlang: al
ms.search.keywords: service contracts, contract quotes, annual amount
ms.date: 10/03/2025
ms.author: bholtorf
...
## Change the annual amount of the service contract or contract quote  
...
5. Change the contents of the **Annual Amount** field. You can't sign, that is, convert into a service contract if you're working on a contract quote, or lock a service contract that has a negative annual amount. If you set the annual amount to zero, the contents of the **Invoice Period** field must be **None** when either signing or locking the service contract.  
6. Depending on whether the **Allow Unbalanced Amounts** check box is selected, run either the manual or automatic distribution of the annual amount difference. The contract lines are updated so that the **Calcd. Annual Amount** field value is equal to the new annual amount.  

## Distribute differences between new and calculated annual amounts
...
1. The difference between the new **Annual Amount** and **Calcd. Annual Amount** field values are divided by the number of the contract lines in the service contract or contract quote.  
...
The **Allow Unbalanced Amounts** check box isn't selected in the service contract that contains three contract lines with such information.  
...
The **Annual Amount** field value is equal to the contents of the **Calcd. Annual Amount** field, which is always set to the sum of the line amounts. In this case, it's equal to the following:  40 + 45 + 63 = 148.  

If you change the **Annual Amount** to 139, the amount is calculated that should be added to each **Line Amount** field value. This amount is calculated by subtracting the **Calcd. Annual Amount** from the new **Annual Amount** field value and dividing the result by the number of the contract lines in the service contract. In this case, it's equal to the following: (139 - 148) / 3 = -3. Then, the last calculated figure is added to each **Line Amount** field value and the **Line Discount %**, **Line Discount Amount**, and **Profit** field values are updated using the formulas in the procedure described above.  

Finally, the contract lines contain this data.  
...
If you change the annual amount of the service contract or contract quote, you may want to distribute the difference between its new and calculated annual amounts on the contract lines. Distribution Based on Line Amount is an automatic method that can help you spread the new and calculated annual amounts difference between the line amounts on the contract lines. This distribution is performed proportionally to their line amount shares in the calculated annual amount. The following list of distribution procedure steps for each contract line describe the main idea of this method:  
...
The **Annual Amount** field value is equal to the contents of the **Calcd. Annual Amount** field, which is always set to the sum of the line amounts. In this case, it's equal to the following: 16.49 + 23.00 + 26.19 = 65.68.  

If you change the **Annual Amount** to 60, the profit percentage contribution for each contract line is calculated:  
...
If you change the annual amount of the service contract or contract quote, you may want to distribute the difference between its new and calculated annual amounts on the contract lines. Distribution Based on Profit is one of the automatic methods that can help you spread the new and calculated annual amounts difference between the line amounts on the contract lines. This distribution is performed proportionally to their profit shares in the total contract or contract quote profit. The following list of distribution procedure steps for each contract line describe the main idea of this method:  
...
The **Annual Amount** field value is equal to the contents of the **Calcd. Annual Amount** field, which is always set to the sum of the line amounts. In this case, it's equal to the following:  25.00 + 55.10 + 112.70 = 192.80.  
