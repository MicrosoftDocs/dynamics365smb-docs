...
|**Bidirectional Synch of Sales Orders**|Synchronize sales orders in both directions. For example, if a customer changes their mind about the product or quantity they ordered in [!INCLUDE[crm_md](includes/crm_md.md)], you can process the change in [!INCLUDE[prod_short](includes/prod_short.md)] by archiving the sales document and creating a new one. The same is true for changes in [!INCLUDE[prod_short](includes/prod_short.md)]. For example, when prices, tax amounts, or expected shipment dates change the changes are automatically synchronized to [!INCLUDE[crm_md](includes/crm_md.md)]. This helps keep your sellers up to date with the latest changes and the status of quotes and orders.|
