...
This article describes the necessary settings and steps that you must perform to synchronize and fulfill sales orders from Shopify in [!INCLUDE[prod_short](../includes/prod_short.md)].
...
- Price including Tax. The **Prices including Tax**/**Prices including VAT** field in the created sales document doesn't depend on the customer, but on the **Customer Template** from the Shopify Shop Card or Customer template per country.
...
|In [!INCLUDE[prod_short](../includes/prod_short.md)], change the location to another location, mapped to the Shopify Locations. Post shipment. | After synchronizing the fulfillment, location will be updated in Shopify. |
|In [!INCLUDE[prod_short](../includes/prod_short.md)], change the location to another location, not mapped to the Shopify Locations. Post shipment. | The fulfillment won't be synchronized to Shopify. |
