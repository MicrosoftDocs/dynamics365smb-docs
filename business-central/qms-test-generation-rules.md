---
title: Set up inspection generation rules
...
Inspection generation rules define when and how to automatically create quality inspections in response to business transactions. These rules connect your quality inspection templates to specific business processes.
...
Inspection generation rules control the following aspects:
...
This section describes the components that make up an inspection generation rule.
...
- **Never**: No automatic inspection creation.
...
3. Fill in the fields, as follows:
   ...
   - **Location, Vendor No., Purchasing Code, Specific Item, Category, Inventory Posting Group**: Specify which items, groups of items, locations, and so on, can create inspections.
   - **Automatically Create Test**: Specify a trigger to automatically create an inspection when you receive a product for a purchase order.

### Create a production rule

This rule is best for inspecting production output.
...
1. [!INCLUDE [prod_short](includes/prod_short.md)], enter **Inspection Generation Rules**, and then choose the related link.
...
|Type of filter  |Use  |
|---------|---------|
|Item Filters |  - **Item No. Filter**: Specific items that require inspection.<br>- **Item Category Filter**: Groups of related items.<br>- **Inventory Posting Group Filter**: Items with similar characteristics.    |
|Location Filters | **Location Code Filter**: Specific locations that require inspections. |
|Vendor/Customer Filters | - **Vendor No. Filter**: Vendor-specific quality requirements.<br>- **Customer No. Filter**: Customer-specific inspection needs. |
|Production Filters     |  - **Routing No. Filter**: Inspect specific production processes.<br>- **Work Center No. Filter**: Inspect specific work centers.<br>- **Machine Center No. Filter**: Inspect specific machines.       |
...
- **All matching rules execute**: Multiple inspections can be created.
...
For reactive inspections, manual triggers provide the following benefits:

- They base inspections on risk assessment.
- They require limited quality resources.
- They make inspections driven by investigations.
...
1. Create an inspection scenario:

   - Configure a rule with specific filters.
   - Create a matching business transaction.
   - Verify that an inspection is created as expected.
...
3. Review the results:
   ...
   - Validate the inspection content and structure.
