...
[!INCLUDE[navnow](../ApplicationDesign/includes/navnow_md.md)] automatically calculates the date on which you must order an item to have it in inventory on a certain date. This is the date on which you can expect items ordered on a particular date to be available for picking.  
  
 If you specify a requested receipt date on a purchase order header, then the calculated order date is the date on which the order must be placed to receive the items on the date that you requested. Then, the date on which the items are available for picking is calculated and entered in the **\($ T\_39\_10 Expected Receipt Date $\)** field.  
...
## Calculating without a Requested Delivery Date  
 If you enter a purchase order line without a requested delivery date, then the **\($ T\_39\_5795 Order Date $\)** field on the line is filled with the date in the **\($ T\_38\_19 Order Date $\)** field on the purchase order header. This is either the date that you entered or the work date. The following dates are then calculated for the purchase order line, with the order date as the starting point.  
 ...
 If you change the order date on the header, then that date is copied to the **\($ T\_39\_5795 Order Date $\)** field on all the lines, and all the related date fields are then recalculated.  
  
## See Also  
 [\($ T\_39\_10 Expected Receipt Date $\)](../Topic/\($%20T_39_10%20Expected%20Receipt%20Date%20$\).md)   
 [\($ T\_39\_5795 Order Date $\)](../Topic/\($%20T_39_5795%20Order%20Date%20$\).md)   
